...
from models.utils import mask_tokens
...
class NLIModel(nn.Module):
    def __init__(self,bert,bert_tokenizer,k = 10,mask_prob = 0.5,num_classes = 50,embedding_dim = 128):
        ...
        self.contrastive_head = nn.Linear(self.bert.config.hidden_size,embedding_dim).to(self.device)
    ...
    def predict(self,input,return_output = False):
        input_tokens = self.bert_tokenizer(input,truncation=True,return_tensors='pt',padding = 'longest',max_length = 512).to(self.device)
    ...
    def forward(self,input,labels = None, k = None):
        ...
        if self.training:
            ...
            input_tokens = self.bert_tokenizer(self.desc,truncation=True,return_tensors='pt',padding = 'longest',max_length = 512).to(self.device)
        ...
        return cls_embeddings,desc_embeddings,torch.Tensor(out_classes).to(self.device)
