...
class ChronosBoltModelForForecasting(T5PreTrainedModel):
    ...
    def __init__(self, config: T5Config,static_dim = None,static_embeddings:list[int] = None,decompose_dim = None,txn_emb = 55,n_inn = None):
        ...
        self.inn_embedding = nn.Embedding(n_inn,config.d_model)
        ...
        self.holiday_norm = nn.LayerNorm(config.d_model)
        self.static_norm = nn.LayerNorm(config.d_model)
        ...
        # self.output_patch_embedding = nn.Linear(config.d_model,3)
        # self.seasonal_patch_embedding = ResidualBlock(
        #     in_dim=config.d_model,
        #     h_dim=config.d_ff,
        #     out_dim=1,
        #     act_fn_name=config.dense_act_fn,
        #     dropout_p=0,
        # )
        # # self.cyclical_patch_embedding = ResidualBlock(
        # #     in_dim=config.d_model,
        # #     h_dim=config.d_ff,
        # #     out_dim=1,
        # #     act_fn_name=config.dense_act_fn,
        ...
        # self.trend_patch_embedding = ResidualBlock(
        #     in_dim=config.d_model,
        #     h_dim=config.d_ff,
        #     out_dim=1,
        #     act_fn_name=config.dense_act_fn,
        ...
        # self.holiday_week = nn.Embedding(2,config.d_model)
        self.holiday = nn.Embedding(3,config.d_model)
    ...
    def forward(
        ...
        holiday_target = self.holiday_norm(holiday_target+target_positions)
        ...
        dhidden_states = x_emb+dhidden_states
        ...
        if decompose_features is not None:
            ...
            decompose_features[...,4],trend_loc_scale = self.instance_norm(decompose_features[...,4])
            decompose_features[...,5],seasonal_loc_scale = self.instance_norm(decompose_features[...,5])
        ...
        if static_features is not None:
            ...
            static = self.static_norm(self.static_embedding(static_features)+self.inn_embedding(client_ids))
        ...
        sequence_output = self.output_patch_embedding(sequence_output)

        
        trend_preds = sequence_output[...,0]
        seasonal_preds = sequence_output[...,1]
        irregular_preds = sequence_output[...,2]
        # cyclical_preds = self.cyclical_patch_embedding(sequence_output).squeeze(-1)

        quantile_preds = irregular_preds+self.instance_norm.inverse(trend_preds,trend_loc_scale)+self.instance_norm.inverse(seasonal_preds,seasonal_loc_scale)

        
        median = quantile_preds

        preds = [trend_preds,seasonal_preds]
        loc_scales = [trend_loc_scale,seasonal_loc_scale]
        

        total_loss = 0
        if decompose_target is not None:
            targets = decompose_target[...,1:]
            losses = [0 for _ in range(len(preds))]
            median = median[...,:decompose_target.shape[1]]
            for t in range(len(preds)):
                    target,_ = self.instance_norm(targets[...,t],loc_scales[t])  # type: ignore
                    ...
                    losses[t] += torch.clamp(loss,max=10)
            ...
            pred = torch.log1p(torch.expm1(torch.relu(torch.where((holiday_target_ids > 0),median,median))).view(*median.shape[:-1],-1,7).sum(-1))
