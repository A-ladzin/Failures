...
class ChronosBoltOutput(ModelOutput):
    ...
    workday_loss: Optional[torch.Tensor] = None
    holiday_loss: Optional[torch.Tensor] = None
    preds: Optional[torch.Tensor] = None
    attentions: Optional[torch.Tensor] = None
    cross_attentions: Optional[torch.Tensor] = None

def huber_loss(pred, target, mask=None, delta=2, halfa = 0.5):
    ...
    abs_diff = diff.abs()
    if mask is None:
        mask = torch.ones_like(target).to(target.device).float()
    else:
        mask = mask.float()
    # mask = torch.where(((pred <= 0 ) * (target <= 0)),mask*1e-2,mask)

    mse = (torch.nn.functional.mse_loss(pred,target,reduction='none')*mask).mean()

    quadratic = torch.where(abs_diff < delta, 0.5 * diff.pow(2), delta * abs_diff - 0.5 * delta**2)
    loss = (quadratic * mask).mean()
    # return loss
    return loss*halfa + mse*(1-halfa)
...
class ChronosBoltModelForForecasting(T5PreTrainedModel):
    ...
    def __init__(self, config: T5Config,static_dim = None,static_embeddings:list[int] = None,decompose_dim = None,txn_emb = 55):
    ...
    def forward(
        ...
        dhidden_states = self.holiday_context_attention(dhidden_states,encoder_input = holiday_context)
        dsequence_output = self.decode(dinputs_embeds,dattention_mask,dhidden_states,decoder = self.decoder_)
        ...
        if decompose_features is not None:
            ...
            sequence_output = self.decoder(inputs_embeds = x_emb,encoder_hidden_states=hidden_states, output_hidden_states = True)[0]

        sequence_output = self.decompose_output_attention(dsequence_output, encoder_input = sequence_output)
        ...
        if decompose_target is not None:
            ...
            # torch.where((median > 0) + (decompose_target[...,0] > 0),1,1e-3))
            forecast_loss = (torch.abs(median-decompose_target[...,0])).mean()
            total_loss+=forecast_loss


            

            # pred_holiday = torch.log1p(torch.expm1(torch.relu(torch.where((holiday_target_ids > 0),median,0))).view(*median.shape[:-1],-1,7).sum(-1))
            # pred_workday = torch.log1p(torch.expm1(torch.relu(torch.where((holiday_target_ids > 0),0,median))).view(*median.shape[:-1],-1,7).sum(-1))

            # holiday_target = torch.log1p(torch.expm1(torch.relu(torch.where((holiday_target_ids > 0),decompose_target[...,0],0))).view(*median.shape[:-1],-1,7).sum(-1))
            # workday_target = torch.log1p(torch.expm1(torch.relu(torch.where((holiday_target_ids > 0),0,decompose_target[...,0]))).view(*median.shape[:-1],-1,7).sum(-1))

            
            preds = torch.log1p(torch.expm1(torch.relu(median)).view(*median.shape[:-1],-1,7).sum(-1))
            holiday_loss = huber_loss(preds,torch.log1p(torch.expm1(torch.relu(decompose_target[...,0])).view(*median.shape[:-1],-1,7).sum(-1)),delta= 2)
            # workday_loss = huber_loss(pred_workday,workday_target,delta= 2)

            total_loss = total_loss +holiday_loss*4e-1
            # total_loss += holiday_loss
            ...
            return ChronosBoltOutput(
                ...
                holiday_loss = holiday_loss,
                workday_loss = holiday_loss,
                preds=preds
            )
        preds = torch.log1p(torch.expm1(torch.relu(median)).view(*median.shape[:-1],-1,7).sum(-1))
        return ChronosBoltOutput(
    ...
    def _init_decoder(self, config):
        ...
        self.final_decoder = T5Stack(decoder_config_, self.shared)
