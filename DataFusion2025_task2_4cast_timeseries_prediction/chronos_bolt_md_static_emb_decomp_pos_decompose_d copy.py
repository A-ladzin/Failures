...
class ChronosBoltModelForForecasting(T5PreTrainedModel):
    ...
    def __init__(self, config: T5Config,static_dim = None,static_embeddings:list[int] = None,decompose_dim = None,txn_emb = 55):
        ...
        if static_dim is not None:
            ...
            h_dim=config.d_ff,
            out_dim=config.d_model*2,
            act_fn_name=config.dense_act_fn,
            dropout_p=config.dropout_rate,use_layer_norm=True
        )
            self.static_fuse = FuseBlock(config.d_model*2,config.d_ff,config.d_model,config.dense_act_fn,use_layer_norm=True)
        ...
        self.holiday_target_attention = TransformerCrossAttentionLayer(
        attn,
        nn.Sequential(
            nn.Linear(config.d_model, config.d_ff),
            nn.GELU(),
        ...
        self.irregular_patch_embedding = ResidualBlock(
            in_dim=config.d_model,
            h_dim=config.d_ff,
    ...
    def forward(
        ...
        holiday_target = self.holiday(holiday[:,decompose_features.shape[1]:]).squeeze(-2)
        ...
        if decompose_features is not None:
            ...
            sequence_output = self.decode(x_emb,attention_mask=None,hidden_states=hidden_states)
        sequence_output = torch.cat((sequence_output,dsequence_output),dim=-1)
        ...
        sequence_output = self.holiday_target_attention(sequence_output,encoder_input = holiday_target)

        irregular_preds = self.irregular_patch_embedding(sequence_output).view(
                *quantile_preds_shape
            )   
        
        trend_preds = self.trend_patch_embedding(sequence_output).view(
                *quantile_preds_shape
            )   
        seasonal_preds = self.seasonal_patch_embedding(sequence_output).view(
                *quantile_preds_shape
            )   
        cyclical_preds = self.cyclical_patch_embedding(sequence_output).view(
                *quantile_preds_shape
            )  
        
        quantile_preds = irregular_preds+trend_preds+seasonal_preds+cyclical_preds

        
        median = quantile_preds[:,4]

        preds = [trend_preds,seasonal_preds,cyclical_preds,irregular_preds]
        

        total_loss = 0
        if decompose_target is not None:
            targets = decompose_target[...,:decompose_target.shape[1]]
            losses = [0 for _ in range(len(preds))]
            median = median[...,:decompose_target.shape[1]]
            for t in range(len(preds)):
                    target = targets[...,t]  # type: ignore
                    ...
                    loss = torch.abs(target - preds[t][:,4,:decompose_target.shape[1]]).mean()
                    losses[t] += loss
                    total_loss += loss
                    

            forecast_loss = (torch.abs(median-decompose_target[...,0])*torch.where((median > 0) + (decompose_target[...,0] > 0),1,1e-2)).mean()
            ...
            return ChronosBoltOutput(
                ...
                irregular_loss=losses[3],
                quantile_preds=pred.squeeze(-1)
            )
        pred = torch.log1p(torch.expm1(torch.relu(median)).view(*median.shape[:-1],-1,7).sum(-1))
        return ChronosBoltOutput(
                pred=median.squeeze(-1)
            )
