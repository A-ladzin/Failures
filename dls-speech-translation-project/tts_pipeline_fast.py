...
class TTSWrapper:
    ...
    """
    def __init__(self, model_name="tts_models/multilingual/multi-dataset/fastspeech2", use_cuda=True):
        self.model_name = model_name
        self.device = torch.device("cuda" if torch.cuda.is_available() and use_cuda else "cpu")
        
        # Initialize TTS (will download model if needed)
        self.tts = TTS(model_name).to(self.device)
        self.tts.config.use_external_duration = True
...
def synthesize_timealigned_segments(
    ...
    for idx, seg in tqdm(enumerate(translated_segments)):
        ...
        target_duration_sec = end-start  # target duration in seconds
        ...
        processor = tts_wrapper.tts.processor
        phoneme_ids = processor.text_to_sequence(ru_txt, inference=True)
        hop_length = tts_wrapper.tts_config["audio"]["hop_length"]       # e.g. 256
        sample_rate = tts_wrapper.tts_config["audio"]["sample_rate"]   
        num_phonemes = len(phoneme_ids)
        seconds_per_frame = hop_length / sample_rate
        per_phoneme_seconds = target_duration_sec / num_phonemes
        per_phoneme_frames = int(round(per_phoneme_seconds / seconds_per_frame))
        durations = np.array([per_phoneme_frames] * num_phonemes, dtype=np.int32)
        total_frames = durations.sum()
        actual_duration = total_frames * seconds_per_frame
        duration_error = target_duration_sec - actual_duration
        if abs(duration_error) > (seconds_per_frame / 2):
            # Compute how many frames we need to add (positive) or remove (negative)
            frame_delta = int(round(duration_error / seconds_per_frame))
            durations[-1] += frame_delta
